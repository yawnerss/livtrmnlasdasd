...
# ---------- Embedded HTML UI (same as before) ----------
HTML_PAGE = """
<!DOCTYPE html>
<html lang="en">
<head>
...
"""  # (keep the exact same HTML from the previous answer)

@app.route('/')
def index():
    return render_template_string(HTML_PAGE)

# ---------- SocketIO Events with error handling ----------
@socketio.on('connect')
...
def handle_register(data):
    try:
        ...
        emit('client_list', {sid: {'name': data['name'], 'metrics': data['metrics']} for sid, data in clients.items()}, broadcast=True)
        print(f"[REGISTER] Client {request.sid} registered as {name}")
    except Exception as e:
...
def handle_new_terminal():
    try:
        ...
        print(f"[NEW_TERMINAL] Creating terminal {session_id} for client {request.sid}")
        ...
        print(f"[ERROR] in new_terminal: {e}\n{traceback.format_exc()}")
        return None

@socketio.on('terminal_ready')
def handle_terminal_ready(data):
    try:
        print(f"[TERMINAL_READY] {data} from client {request.sid}")
    except Exception as e:
        print(f"[ERROR] in terminal_ready: {e}\n{traceback.format_exc()}")
...
def handle_terminal_input(data):
    try:
        ...
        print(f"[ERROR] in terminal_input: {e}\n{traceback.format_exc()}")
...
def handle_terminal_resize(data):
    try:
        ...
        print(f"[ERROR] in terminal_resize: {e}\n{traceback.format_exc()}")
...
def handle_close_terminal(data):
    try:
        ...
        print(f"[ERROR] in close_terminal: {e}\n{traceback.format_exc()}")
...
def handle_terminal_output(data):
    try:
        ...
        print(f"[ERROR] in terminal_output: {e}\n{traceback.format_exc()}")
